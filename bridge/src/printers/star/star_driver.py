...
This module implements the BasePrinter interface for Star fiscal printers.
Uses MHI fiscal protocol (same as CTS310ii).

Note: Star printers use the same MHI protocol as CTS310ii, so this driver
...
class StarDriver(BasePrinter):
    """
    Star Fiscal Printer Driver.

    Implements the BasePrinter interface for Star fiscal printers.
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        logger.info(f"Star driver initialized (debug={self.debug})")
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            configured_port = self.config.get('printer', {}).get('star', {}).get('com_port')
            ...
            if configured_port and configured_port.upper() not in ['AUTO', 'NULL', 'NONE']:
                ...
                if self._is_success_response(response):
                    logger.info(f"Found Star printer on configured port {self.com_port}")
                    self.connected = True
            ...
            logger.info("Scanning for Star printer...")
            ...
            for port in reversed(ports):
                ...
                if self._is_success_response(response):
                    logger.info(f"Found Star printer on {self.com_port}")
                    self.connected = True
            ...
            logger.error("Star printer not found on any COM port")
            return False
