...
This module implements the BasePrinter interface for Citizen fiscal printers.
Uses MHI fiscal protocol (same as CTS310ii).

Note: Citizen printers use the same MHI protocol as CTS310ii, so this driver
is functionally identical to the CTS310ii driver with only naming differences.
...
class CitizenDriver(BasePrinter):
    """
    Citizen Fiscal Printer Driver.

    Implements the BasePrinter interface for Citizen fiscal printers.
    Supports fiscal document printing, X/Z reports, and document management.

    Note: Uses MHI fiscal protocol (same as CTS310ii).
    """

    def __init__(self, config: Dict[str, Any]):
        """Initialize the Citizen driver.
        ...
        logger.info(f"Citizen driver initialized (debug={self.debug})")
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            configured_port = self.config.get('printer', {}).get('citizen', {}).get('com_port')
            ...
            if configured_port and configured_port.upper() not in ['AUTO', 'NULL', 'NONE']:
                ...
                if self._is_success_response(response):
                    logger.info(f"Found Citizen printer on configured port {self.com_port}")
                    self.connected = True
            ...
            logger.info("Scanning for Citizen printer...")
            ...
            for port in reversed(ports):
                ...
                if self._is_success_response(response):
                    logger.info(f"Found Citizen printer on {self.com_port}")
                    self.connected = True
            ...
            logger.error("Citizen printer not found on any COM port")
            return False
    ...
    def print_document(
        ...
        try:
            ...
            if doc_type in ["1", "3"]:
                ...
                # Use spaces for CRIB to hide it on receipt
                fiscal_customer_name = default_customer_name
                fiscal_customer_crib = " " * 10  # Spaces to hide CRIB NUMBER/ID CONSUMER line
            else:
    ...
    def print_z_report_by_number_range(self, start: int, end: int) -> Dict[str, Any]:
        ...
        try:
            ...
            start_hex = string_to_hex(str(start).zfill(4))
            end_hex = string_to_hex(str(end).zfill(4))
    ...
    def print_no_sale(self) -> Dict[str, Any]:
        ...
        Prints a fiscal training document (mode 1) with NO SALE text.
        Training mode documents don't affect fiscal totals.
        ...
        try:
            ...
            logger.info("Printing No Sale document")

            # Get defaults
            default_customer_crib = self.misc_config.get("default_client_crib", "1000000000")

            # Generate dynamic NKF for NO SALE document (type 1)
            nkf_config = self.client_config.get("NKF", {})
            ...
            client_nkf = generate_nkf(nkf_config, "1", last_order_id)

            # Cancel any open document first
            self._cancel_document()
            ...
            fiscal_object = {
                ...
                "customer_CRIB": default_customer_crib,
                "NKF": client_nkf,
                "NKF_affected": client_nkf,
            }
            ...
            document_number = self._prepare_document(fiscal_object)
            if not document_number:
                return {"success": False, "error": "Failed to prepare NO SALE document"}

            # Add NO SALE message as comments
            self._add_comment("=" * 48)
            self._add_comment("")
            self._add_comment("          NO SALE          ")
            self._add_comment("")
            self._add_comment("=" * 48)
            ...
            subtotal = self._document_sub_or_total("0")
            if not subtotal:
                return {"success": False, "error": "Failed to process NO SALE subtotal"}

            # Calculate TOTAL (0.00)
            total = self._document_sub_or_total("1")
            if not total:
                return {"success": False, "error": "Failed to process NO SALE total"}

            # Process a 0 payment to close
            payment_data = {
                "type": "1",
                "method": "01",  # Cash
                "description": " ",
            ...
            if not self._payment(payment_data):
                return {"success": False, "error": "Failed to process NO SALE payment"}

            # Close the document
            close_result = self._close_document("NO SALE completed")
            if close_result:
